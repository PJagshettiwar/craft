...
- `bash "${CRAFT_HOME:-.}/scripts/craft-doctor.sh" --tests`

The last one asks a question a green suite cannot answer: **did the suite get weaker while
the feature got built?** It reports deleted test files, new skip markers, dropped assertion
counts, and test files containing no assertion at all.

It never blocks — a legitimate refactor can lower an assertion count. But every line it prints
needs one sentence in your review saying why it is fine. An unexplained `TEST-WEAKENED` is an
Important finding, and `TEST-DELETED` without a spec change behind it is Critical: deleting a
test is a change to what the system promises.
...
<real logic or just mocks? edge cases? any test that asserts nothing or would pass
 with the implementation reverted? Include the --tests output below.>
